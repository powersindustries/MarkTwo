...
{
    TextureAssetData& iconTextureData = m_TextureAssets[CoreSystems::StringToHash32(AssetID.c_str(), AssetID.length())];
...
void AssetManager::LoadTextureAssets(SDL_Renderer* renderer)
{
    ...
    for (rapidxml::xml_node<>* child = texturesNode->first_node(); child; child = child->next_sibling())
    {
        ...
        m_TextureAssets[CoreSystems::StringToHash32(textureAssetData.m_ID.c_str(), textureAssetData.m_ID.length())] = textureAssetData;
    }
...
void AssetManager::LoadFontAssets(SDL_Renderer* renderer)
{
    ...
    for (rapidxml::xml_node<>* child = fontNode->first_node(); child; child = child->next_sibling())
    {
        ...
        m_FontAssets[CoreSystems::StringToHash32(fontAssetData.m_ID.c_str(), fontAssetData.m_ID.length())] = fontAssetData;
    }
...
void AssetManager::LoadSoundAssets()
{
    ...
    for (rapidxml::xml_node<>* child = soundNode->first_node(); child; child = child->next_sibling())
    {
        ...
        {
            m_SoundAssets[CoreSystems::StringToHash32(soundAssetData.m_ID.c_str(), soundAssetData.m_ID.length())] = soundAssetData;
        }
        else
        {
            ...
            CoreSystems::SYSTEMS_LOG(CoreSystems::LoggingLevel::eError, errorMessage.c_str());
        }
    }
...
void AssetManager::LoadMusicAssets()
{
    ...
    for (rapidxml::xml_node<>* child = musicNode->first_node(); child; child = child->next_sibling())
    {
        ...
        {
            m_MusicAssets[CoreSystems::StringToHash32(musicAssetData.m_ID.c_str(), musicAssetData.m_ID.length())] = musicAssetData;
        }
        else
        {
