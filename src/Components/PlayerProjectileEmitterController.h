...
namespace MarkTwo
{
    namespace GameCore
    {
        class PlayerProjectileEmitterComponent : public Component
        {
        public:
            PlayerProjectileEmitterComponent(int speed, int angleDeg, int range, bool shouldLoop)
            {
                m_Speed = speed;
                m_Range = range;
            ...
            {
                if (m_bShooting && glm::distance(m_Transform->position, m_OriginPoint) > m_Range)
                {
                    if (m_bShouldLoop)
                    {
